...
app.get('/api/getCompagnon/:pseudo',(req,res)=>{
    const pseudo = req.params.pseudo;
    db.query("SELECT pseudo, pokemon, level, xp FROM compagnon WHERE pseudo = ? AND actif = 1", pseudo, (err,result)=>{
        if(err) {
            console.log(err)   }
        res.send(result)
    });
...
app.get("/api/getProfil/:pseudo", (req, res, next)=>{
    ...
    const pseudo = req.params.pseudo;
    db.query("SELECT pseudo, xp, first_pokemon, second_pokemon, third_pokemon, fourth_pokemon, fifth_pokemon, sixth_pokemon, profil_picture, level, box, canOpen, lastOpening, pkmToken, badge, powder FROM profil WHERE pseudo = ?", pseudo,
        (err,result)=>{
            if(err) {
                console.log(err)
            }
